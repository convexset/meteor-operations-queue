...
var OperationsQueue = require('../src');
...
function checkOutput(name, expectedOutput) {
	return function(output) {
		...
		expect(thingToCheck).to.equal(expectedOutput);
	};
}

function completionCallBack(name, done, data) {
	return function(completed, incomplete) {
		...
		precedences.forEach(function(tc) {
			tc.dependencies.forEach(function(depId) {
				...
				expect(taskData.taskCompletionTime[depId]).to.be.at.most(taskData.taskStartTime[tc.task]);
			});
		});
		...
		while ((sIdx < startTimes.length) || (cIdx < completionTimes.length)) {
			...
			_.forEach(resourceAvailability, function(v, k) {
				expect(currRes[k]).to.be.at.least(0);
			});

			if ((sIdx >= startTimes.length - 1) && (cIdx >= completionTimes.length - 1)) {
				_.forEach(resourceAvailability, function(v, k) {
					expect(currRes[k]).to.equal(v);
				});
...
describe('Trial by random instances', function() {
	_.times(10, function(trialIdx) {
		it(`Completion, Precedences and Resource Feasibility. Trial #${trialIdx}`, function(done) {
			...
			var opsQueue = new OperationsQueue({
				...
				terminationCallBack: completionCallBack("The Example", done, data)
			});
			data.opsQueue = opsQueue;

			var resUse, deps;

			(function prepareBaseExample() {
				// The Example
				resUse = {
					x: 1,
					y: 1
				...
				var t11 = opsQueue.createTask({
					...
					taskCompletionCallBack: checkOutput("t11", 1)
				});
				...
				var t12 = opsQueue.createTask({
					...
					taskCompletionCallBack: checkOutput("t12", 10)
				});
				...
				var t2 = opsQueue.createTask({
					...
					taskCompletionCallBack: checkOutput("t2", 11)
				});
				precedences.push({
					task: t2,
					dependencies: deps
				...
				var t13 = opsQueue.createTask({
					...
					taskCompletionCallBack: checkOutput("t13", 100)
				});
				...
				var t3 = opsQueue.createTask({
					...
					taskCompletionCallBack: checkOutput("t3", 111)
				});
				precedences.push({
					task: t3,
					dependencies: deps
			...
			(function prepareRandomizedPartOfExample() {
				var NUM_LINES = 5;
				var NUM_ITEMS = 5;
					...
					for (item = 1; item <= NUM_ITEMS; item++) {
						...
						taskIdsSerialLines[line][item] = opsQueue.createTask({
							...
							taskCompletionCallBack: (function(ln) {
								return checkOutput(taskName, ln);
							})(line)
						});
						precedences.push({
					...
					for (item = 1; item <= NUM_ROW_ITEMS; item++) {
						...
						taskIdsCoupled[row][item] = opsQueue.createTask({
							...
							taskCompletionCallBack: (function(ans) {
								return checkOutput(taskName, ans);
							})(taskName)
						});
						precedences.push({
